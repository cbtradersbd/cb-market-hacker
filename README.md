# ⚡ Quotex 24/7 Live REST API & Signal Engine

> **Production-grade 24/7 self-healing REST API & WebSocket wrapper for Quotex (Real & OTC asset pairs).**
...
- 🕒 **24/7 Real & OTC Coverage**: Real-time market data streaming for Quotex.
...
| `/api/quotex/live-price` | `GET` | Real-time price stream & tick updates |
| `/api/quotex/candles` | `GET` | Closed M1/M5 historical candle datasets |
| `/api/quotex/payouts` | `GET` | Live payout percentage monitor across all pairs |
| `/api/quotex/signals` | `POST` | Strategy webhook trigger & alert dispatcher |
